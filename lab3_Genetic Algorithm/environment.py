...
class Environment:
    def __init__(self, num_slot, num_students):
        ...
        self.slot_priorities = np.random.randint(1, 6, size=num_slot)
...
def fitness(individual, environment):
    ...
    for task, student_id in enumerate(individual):
        # Check for conflict
        if environment.days[task % len(environment.days)] != environment.students_days[student_id]:
            conflict_penalty += 1

        # Calculate preference penalty
        preference_penalty += 1 / (task + 1)  # Example: Adjust penalty for slot preference
